...
serve(async (req) => {
  ...
  try {
    ...
    const grammarScore = analysisResults[0].result[0][1];
    if (grammarScore < 0.8) {
      suggestions.push("Consider reviewing your grammar and sentence structure.");
    }

    // Process toxicity analysis
    const toxicityScore = analysisResults[1].result[0].score;
    ...
    const styleScore = analysisResults[2].result[0][1];
    if (styleScore < 0.7) {
    ...
    const structureScore = analysisResults[3].result[0][1];
